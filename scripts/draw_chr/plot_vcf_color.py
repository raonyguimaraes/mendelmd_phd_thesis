...
#define four colors
colors={'A':'red', 'C':'black', 'G':'blue','T':'green'}



#given a vcf file return list of homozygous variants in radians
def vcf_rad_dict(vcffile):
    vcffile = open(vcffile, 'r')
    variants_rad_dict = {'A':[], 'C':[], 'G':[], 'T':[]}
    for line in vcffile.readlines():
        if not line.startswith('#'):
            variant = line.strip().split('\t')
            chr = variant[0].replace('chr', '')
            ...
            # if variant[2] == '.':
                if variant[6] == 'PASS':
                    ...
                    # variants_rad_list.append(rad_pos)
                    if variant[4][0] == 'A':
                        variants_rad_dict['A'].append(rad_pos)
                    if variant[4][0] == 'C':
                        variants_rad_dict['C'].append(rad_pos)
                    if variant[4][0] == 'G':
                        variants_rad_dict['G'].append(rad_pos)
                    if variant[4][0] == 'T':
                        variants_rad_dict['T'].append(rad_pos)
                        


    return variants_rad_dict
    
def plot_vcf(vcffile):
    ymin = 8
    ymax = ymin + 0.8
    # variants_rad_list = vcf_rad_list(vcffile)
    variants_rad_dict = vcf_rad_dict(vcffile)

    print 'plot vcf'
    # plt.vlines(variants_rad_list, ymin, ymax, linewidth=0.1, color='blue',antialiased=True)
    #plot four colors
    plt.vlines(variants_rad_dict['A'], ymin, ymax, linewidth=0.1, color=colors['A'], antialiased=False)
    ymin = 7
    ymax = ymin + 0.8
    plt.vlines(variants_rad_dict['C'], ymin, ymax, linewidth=0.1, color=colors['C'], antialiased=False)
    ymin = 6
    ymax = ymin + 0.8

    plt.vlines(variants_rad_dict['G'], ymin, ymax, linewidth=0.1, color=colors['G'], antialiased=False)
    ymin = 5
    ymax = ymin + 0.8

    plt.vlines(variants_rad_dict['T'], ymin, ymax, linewidth=0.1, color=colors['T'], antialiased=False)

# plot_vcf('/home/raony/Doutorado/phd-thesis/data/family/Exome_7_EUF.var.annotated.vcf', 8)
# plot_vcf('/home/raony/Doutorado/phd-thesis/data/family/Exome_8_MF.var.annotated.vcf', 7)    
plot_vcf('/home/raony/Doutorado/phd-thesis/data/family/exome_3_eds.var.annotated.vcf')

# plot_vcf('/home/raony/Doutorado/phd-thesis/data/family/exome_4_els.var.annotated.vcf', 5)
# plot_vcf('/home/raony/Doutorado/phd-thesis/data/family/exome_5_ls.var.annotated.vcf', 4)
# plot_vcf('/home/raony/Doutorado/phd-thesis/data/family/exome_6_dc.var.annotated.vcf', 3)
